...
export class ClientUsersService implements OnModuleInit {
  ...
  async setUserStatus(id: string, targetStatus: ClientUserStatus, user: JwtPayload): Promise<ClientUser> {
    ...
    if (snapshot.status === targetStatus) {
      return this.mapToDto(snapshot, client);
    }
    ...
    try {
      ...
      if (completedRun.status === 'FAILED' || completedRun.status === 'TIMED_OUT') {
        ...
        // HTTP 409 for verification unknown (mutation submitted, remote status unknown)
        if (errorCode === 'REMOTE_STATUS_VERIFICATION_UNKNOWN' || errorCode === 'MUTATION_SUBMITTED_VERIFICATION_PENDING') {
          throw new ConflictException({
            code: 'REMOTE_STATUS_VERIFICATION_UNKNOWN',
            message: 'Status action may have completed, but verification is pending. No automatic second click was performed. Use Refresh Current Status before retrying.',
